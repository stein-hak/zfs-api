        ...
        if len(length_data) == 4:
            ...
            try:
                ...
                if resp_data.get("status") == "started":
                    ...
                    # The server is now ready for zfs send data
                    # For testing, just close the connection
                    sock.close()
                    print("✓ Connection closed")
                elif resp_data.get("status") == "failed":
                    print(f"❌ Authentication failed: {resp_data.get('error')}")
                    sock.close()
                else:
        ...
        if len(length_data) == 4:
            ...
            try:
                ...
                if resp_data.get("status") == "started":
                    ...
                    print(f"  Operation: {resp_data.get('operation')}")
                    print(f"  Dataset: {resp_data.get('dataset')}")
                    # The server is now ready for zfs receive data
                    # For testing, just close the connection
                    sock.close()
                    print("✓ Connection closed")
                elif resp_data.get("status") == "failed":
